...
DialogueData Game::LoadDialogue(std::string_view file) {
	...
	if (!doc.load_file(file.data())) {
		throw std::runtime_error("Failed to load dialogue file: " + std::string(file));
	}
	for (const auto& character : doc.child("dialogue").children("character")) {
		DialogueMeta meta;
		meta.character_name = character.attribute("name").as_string();
    meta.character_image = ResourceManager::GetTexture(character.attribute("texture").as_string()).texture;
...
void Game::SaveDialogue(const DialogueData& dialogue, std::string_view file) {
	...
	if (!doc.save_file(file.data())) {
		throw std::runtime_error("Failed to save dialogue file: " + std::string(file));
	}
}
...
void Game::RenderDialogue(DialogueData& dialogue) {
  ...
  if (dialogue.state.current_line >= dialogue.data.size()) {
    std::print("Warn: Dialogue current_line out of bounds, setting to last line\n");
  ...
  if (meta.character_image) {
    meta.character_image->Render(
			  texture_shader,
        CalculateModelMatrix(glm::vec2{50.0f, window.height * 0.25f},
                             0.0f, glm::vec2(100.0f, 100.0f))
		);
	}
  ...
  for (auto& line : meta.dialogue_lines) {
		font->RenderUI(
			line,
			glm::vec2{200.0f, window.height * 0.25f},
			glm::vec2(1.0f),
			glm::vec3(1.0f),
			font_shader);
	}
