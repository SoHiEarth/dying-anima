...
void game::Log::LoadLog(pugi::xml_node& node) {
  ...
  for (auto& entry : entries) {
    ...
    log_entry.texture = resource_manager::GetTexture(
...
void game::Log::SaveLog(pugi::xml_node& node) const {
  ...
  for (const auto& entry : log_) {
    ...
    for (const auto& [level, title] : entry.title) {
      ...
      degradation_node.append_attribute("level") = static_cast<int>(level);
...
void game::Log::RenderLog(DegradationLevel degrade_level) {
  ...
  auto size = ImVec2(GetGameWindow().width - (kLogMargin * 2),
                     GetGameWindow().height - (kLogMargin * 2));
  ...
  if (ImGui::BeginChild("EntryList",
                        ImVec2(ImGui::GetContentRegionAvail().x / 3.0F, 0))) {
  ...
  if (selected_log_index >= 0 &&
    ...
    if (ImGui::BeginChild("EntryDetails", ImVec2(0, 0))) {
      ...
      ImGui::Image(entry.texture.texture ? entry.texture.texture->id : 0,
                   ImVec2(ImGui::GetWindowSize().x, 200), IMGUI_TEXTURE_FLIP);
      ImGui::Separator();
      ImGui::Text("%s", entry.description.at(degrade_level).c_str());
      ImGui::EndChild();
    }
  }
  ImGui::End();
