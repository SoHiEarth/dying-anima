...
std::vector<Object> LoadLevel(std::string_view filename,
  ...
  while (std::getline(file, line)) {
    ...
    auto body = registry.emplace<PhysicsBody>(entity, b2CreateBody(physics_world, &box_body_def)).body;
    b2Polygon box_shape = b2MakeBox(transform.scale.x / 2.0f, transform.scale.y / 2.0f);
