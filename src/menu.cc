...
AppState Menu(GameWindow window) {
  ...
  while (!glfwWindowShouldClose(window.window) && state == AppState::MENU) {
    ...
    banner_texture.Render(
        shader, projection, glm::mat4(1.0f),
...
#else
    int text_width = font.GetWidth("Play", 1.0f) + padding +
                     font.GetWidth("Quit", 1.0f) + padding +
                     font.GetWidth("Level Editor", 1.0f);
#endif
    ...
    font.Render("Play", glm::vec2(x_pos, menu_y), 1.0f, glm::vec3(1.0f),
    ...
    font.Render("Quit", glm::vec2(x_pos, menu_y), 1.0f, glm::vec3(1.0f),
                text_shader, projection);
    ...
    font.Render("Level Editor", glm::vec2(x_pos, menu_y), 1.0f, glm::vec3(1.0f),
                text_shader, projection);
#endif
