...
void CheckFileOpen() {
  if (!opened_file) {
    ...
    output_file.open(core::path::GetTempPath() / "log/runtime.log",
                     std::ios::out);
    if (!output_file.is_open()) {
      std::cout << FormatMessage(
          std::format("Failed to open output file {}",
                      (core::path::GetTempPath() / "log/runtime.log").string()),
          "Log");
    }
    std::cout << FormatMessage(
        std::format("Opened output file {}",
                    (core::path::GetTempPath() / "log/runtime.log").string()),
        "Log");
    opened_file = true;
  }
...
core::Exception core::Error(std::string_view content, std::string_view caller) {
  ...
  return Exception(std::format("{:%H:%M:%S}", std::chrono::system_clock::now()),
                   content, caller);
}
